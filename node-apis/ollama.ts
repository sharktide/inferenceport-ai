...
function register(): void {
  ...
  ipcMain.handle("ollama:run", async (_event: unknown, modelName: string): Promise<string> => {
    return new Promise((resolve, reject) => {
  ...
  ipcMain.handle("ollama:delete", async (_event: unknown, modelName: string): Promise<string> => {
    return new Promise((resolve, reject) => {
      exec(`ollama rm ${modelName}`, (err: Error | null, stdout: string, stderr: string) => {
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          ...
          event.sender.send("ollama:chat-error", `JSON parse error: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }
  ...
  ipcMain.handle("ollama:pull", (_event: unknown, modelName: string): Promise<string> => {
    return new Promise((resolve, reject) => {
  ...
  ipcMain.handle("sessions:save", (_event: unknown, sessions: Record<string, unknown>) => saveSessions(sessions));
}
