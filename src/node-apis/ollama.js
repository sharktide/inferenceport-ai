...
async function issueProxyToken() {
    ...
    const res = await fetch("https://dpixehhdbtzsbckfektd.supabase.co/functions/v1/issue-token", {
        method: "POST",
        headers: {
...
function renderBar(completed = 0, total = 0, width = 20) {
    if (!total)
        return "[                  ]";
...
function renderProgress(sections) {
    ...
    for (const section of sections.values()) {
        ...
        lines.push(`${section.label} ${bar} ${pct}% (${formatBytes(completed)} / ${formatBytes(total)})`);
    }
    return lines.join("\n");
}
...
function isPortOpen(port, host = "127.0.0.1", timeout = 2000) {
...
export async function serve() {
    ...
    return new Promise((resolve, reject) => {
        exec(`"${ollamaPath}" serve`, (err, stdout, stderr) => {
            if (err) {
                err.message += `: ${stderr}`;
                reject(err);
            }
...
export async function fetchSupportedTools() {
    const response = await fetch("https://cdn.jsdelivr.net/gh/sharktide/inferenceport-ai@main/MISC/prod/toolSupportingModels.json");
    if (!response.ok) {
    ...
    if (Array.isArray(data) && data.every((name) => typeof name === "string")) {
        ...
        Array.isArray(data.supportsTools) &&
        data.supportsTools.every((name) => typeof name === "string")) {
    ...
    await ensureDir();
    const writeTask = fs.promises.writeFile(dataFilePath, JSON.stringify({ supportsTools }, null, 2), "utf-8");
...
function messagesForModel(history) {
...
function LOG(trace, label, ...args) {
    const time = new Date().toISOString();
    console.log(`[${time}] [${trace}] ${label}`, ...args);
}
function LOG_ERR(trace, label, ...args) {
    const time = new Date().toISOString();
...
export default function register() {
    ...
    ipcMain.handle("ollama:get-server-logs", async () => {
        return await getServerLogs();
    });
    ipcMain.handle("ollama:start-proxy-server", async (_event, port = 52458, users = []) => {
    ...
    ipcMain.handle("ollama:auto-name-session", async (_event, model, prompt, clientUrl) => {
        try {
...
Keep it under 5 words.
    ...
    ipcMain.handle("ollama:list", async (_event, clientUrl) => {
        if (clientUrl) {
    ...
    ipcMain.handle("ollama:run", async (_event, modelName) => {
        return new Promise((resolve, reject) => {
            ...
            `"${ollamaPath}" run ${modelName}`, (err, stdout) => {
                if (err)
                    return reject(err);
                resolve(stdout);
            });
    ...
    ipcMain.handle("ollama:delete", async (_event, modelName, clientUrl) => {
        if (clientUrl) {
            const base = clientUrl.replace(/\/$/, "");
            console.log(base);
    ...
    ipcMain.on("ollama:chat-stream", async (event, modelName, userMessage, search, imageGen, clientUrl) => {
        ...
        let tools = [];
        if (search)
            tools.push(availableTools[0]);
        if (imageGen)
            tools.push(availableTools[1]);
        ...
        try {
            ...
            abortIfNeeded();
            const finalizedToolCalls = [...toolCallBuffer.entries()].map(([index, data]) => ({
                id: `call_${index}`,
                type: "function",
                function: {
                    name: data.name,
            ...
            if (finalizedToolCalls.length) {
                ...
                for (const toolCall of finalizedToolCalls) {
                    ...
                    if (toolCall.function.name === "generate_image") {
                        const { dataUrl } = await GenerateImage(args.prompt, args.width, args.height);
    ...
    ipcMain.handle("ollama:pull", (_event, modelName, clientUrl) => {
    ...
    ipcMain.handle("ollama:start-log-streaming", (event) => {
