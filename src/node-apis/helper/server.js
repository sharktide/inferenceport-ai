...
const VERIFY_URL = "https://dpixehhdbtzsbckfektd.supabase.co/functions/v1/verify_token_with_email";
...
function maskToken(token, visibleChars = 6) {
    if (!token)
        return "undefined";
...
function forwardRequest(req, res) {
    ...
    req.on("end", () => {
        ...
        logLine("INFO", `[Proxy] Forwarding -> ${sanitizeForLog(req.method || "UNKNOWN")} ${sanitizeForLog(targetUrl.href)}`);
        ...
        const proxyReq = http.request(targetUrl, { method: req.method, headers: cleanHeaders }, (proxyRes) => {
            const resChunks = [];
            ...
            proxyRes.on("end", () => {
                ...
                logLine("INFO", `[Proxy] Response <- ${proxyRes.statusCode}`);
            });
...
function verifyToken(token, emails, callback) {
    if (!token)
...
export function startProxyServer(port = 52458, allowedUsers = []) {
    ...
    server = http.createServer((req, res) => {
        ...
        logLine("INFO", `[Connection] From=${hashIP(ip)} Path=${sanitizeForLog(req.url || "/")}`);
        ...
        verifyToken(token, allowedUsers.map(u => u.email), (status, result) => {
            ...
            if (!matched) {
                logLine("WARN", `Authenticated but not allowed: ${sanitizeForLog(maskToken(result.email))}`);
                res.writeHead(403, { "Content-Type": "application/json" });
                return res.end(JSON.stringify({ error: "Access denied" }));
            }
            logLine("INFO", `[Auth] Verified user ${hashIP(sanitizeForLog(result.email))}`);
            ...
            if (role !== "admin") {
                ...
                if (method !== "GET" && sensitivePattern.test(pathValue)) {
                    logLine("WARN", `Access denied ${method} ${sanitizeForLog(pathValue)} role=${role}`);
                    res.writeHead(403, { "Content-Type": "application/json" });
...
export function startLogStreaming(event) {
    if (logWatcher)
        return;
    try {
    ...
    logWatcher = fs.watch(logFile, async () => {
        try {
            ...
            stream.on("end", () => {
                lastSize = stat.size;
                if (chunk)
                    event.sender.send("ollama:logs-append", chunk);
            });
        }
        catch { }
    });
