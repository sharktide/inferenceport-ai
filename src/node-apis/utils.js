...
function detailsBlock(md) {
    md.block.ruler.before("paragraph", "details_block", (state, startLine, endLine, silent) => {
...
export default function register() {
    ...
    ipcMain.handle("utils:web_open", async (_event, url) => {
        shell.openExternal(url);
    });
    ipcMain.on("utils:markdown_parse_and_purify", (event, markdown) => {
        try {
    ...
    ipcMain.on("utils:DOMPurify", (event, html) => {
        try {
            ...
            event.returnValue = `<p>Error cleaning HTML: ${err instanceof Error ? err.message : String(err)}</p>`;
        }
    });
    ipcMain.handle("utils:saveFile", async (_event, filePath, content) => {
        const base = app.getPath("userData");
    ...
    ipcMain.handle("utils:get-hardware-performance-warning", async (_event, modelSizeRaw) => {
