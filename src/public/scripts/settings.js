...
document.getElementById("del-ps").addEventListener('click', showDelModal);
async function showDelModal() {
    ...
    try {
        ...
        if (session.user.app_metadata?.provider === "email") {
            document.getElementById('delete-password-dialog').style.display = 'flex';
            return;
        }
        else {
            const result = await window.auth.deleteAccount();
...
const hostEmailsInput = document.getElementById('host-emails');
const hostStartBtn = document.getElementById('host-start');
const hostStopBtn = document.getElementById('host-stop');
const hostStatus = document.getElementById('host-status');
const RESTART_REQUIRED_KEY = "host_restart_required";
function setHostingUIRunning(running, port) {
    if (hostStartBtn)
        hostStartBtn.disabled = running;
    if (hostStopBtn)
        hostStopBtn.disabled = !running;
    if (hostStatus)
        hostStatus.textContent = running ? `Server running on port ${port}` : 'Server stopped';
}
// Server logs UI wiring
const serverLogsPanel = document.getElementById('server-logs-panel');
const logsRefreshBtn = document.getElementById('logs-refresh');
const logsStartBtn = document.getElementById('logs-start');
const logsStopBtn = document.getElementById('logs-stop');
const logsClearBtn = document.getElementById('logs-clear');
const logsOutput = document.getElementById('logs-output');
let isLogStreaming = false;
function setServerLogUIRunning(running, port) {
    if (!serverLogsPanel)
        return;
    ...
    if (!running) {
        ...
        if (logsStartBtn)
            logsStartBtn.disabled = false;
        if (logsStopBtn)
            logsStopBtn.disabled = true;
    }
}
...
logsStartBtn?.addEventListener('click', async () => {
    ...
    try {
        ...
        isLogStreaming = true;
        if (logsStartBtn)
            logsStartBtn.disabled = true;
        if (logsStopBtn)
            logsStopBtn.disabled = false;
        // register append handler
        window.ollama.onLogAppend((chunk) => {
            appendLogs(chunk);
        });
    }
    catch (e) {
...
async function isLocalProxyRunning(port, timeout = 1000) {
    try {
...
hostStartBtn?.addEventListener('click', async () => {
    ...
    if (emailsToUse.length === 0) {
        if (hostStatus)
            hostStatus.textContent = 'No valid emails configured.';
        return;
    }
...
hostStopBtn?.addEventListener('click', async () => {
    if (hostStatus)
        hostStatus.textContent = 'Stopping...';
    try {
