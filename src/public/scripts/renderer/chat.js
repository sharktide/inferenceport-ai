...
import { BILLING_PORTAL_URL, buildUpgradePlanCards, escapeSubscriptionHtml, installExternalUrlHandler, normalizeUpgradePlanKey, } from "../helper/subscriptionUpgradeUi.js";
import { mergeLocalAndRemoteSessions, safeCallRemote, isOffline, } from "../helper/sync.js";
...
function isNonSvgImageFile(file) {
    if (isSvgFile(file))
        return false;
    return file.type.startsWith("image/") || isRasterImageFile(file.name);
}
function svgTextToPreviewDataUrl(svgText) {
...
const searchLabel = document.getElementById("search-text");
const imageLabel = document.getElementById("img-text");
const videoLabel = document.getElementById("video-text");
const audioLabel = document.getElementById("audio-text");
const textarea = document.getElementById("chat-input");
const typingBar = textarea.closest(".typing-bar");
const featureWarning = document.getElementById("feature-warning");
const lightningToggleTop = document.getElementById("lightning-toggle-top");
const lightningToggleSidebar = document.getElementById("lightning-toggle-sidebar");
const lightningToggleStatus = document.getElementById("lightning-toggle-status");
const sidebarControls = document.getElementById("sidebar-controls");
const lightningStatus = document.getElementById("lightning-status");
const usagePlanNameEl = document.getElementById("usage-plan-name");
const usagePlanMetaEl = document.getElementById("usage-plan-meta");
const usageResetNoteEl = document.getElementById("usage-reset-note");
...
const previewTier = document.getElementById("usage-panel-tier");
let modal;
let upgradeModal;
let editModal;
...
let searchEngine = currentToolSettings.searchEngines;
...
let usageSyncSource = "local";
let usageSyncedAt = null;
let lastUsageLookupError = null;
let usageSyncTimer = null;
function readLightningSetting() {
    try {
...
function getIsoWeekKey(date = new Date()) {
    ...
    const weekNumber = 1 +
        Math.round(((temp.getTime() - weekOne.getTime()) / 86400000 -
            3 +
            ((weekOne.getDay() + 6) % 7)) /
            7);
...
function getUsageStorageKeyForUser(userId) {
    return `${USAGE_STORAGE_PREFIX}:${userId || "guest"}`;
}
function isKnownPlanKey(value) {
    return (value === "free" ||
        value === "light" ||
        value === "core" ||
        value === "creator" ||
...
function applyTierConfig(tierConfig) {
    if (!tierConfig || !Array.isArray(tierConfig.plans))
        return;
    ...
    tierConfig.plans
        ...
        PLAN_LIMITS[key] = {
            cloudChatDaily: typeof plan.limits?.cloudChatDaily === "number"
                ? plan.limits.cloudChatDaily
                : null,
            imagesDaily: typeof plan.limits?.imagesDaily === "number"
                ? plan.limits.imagesDaily
                : null,
            videosDaily: typeof plan.limits?.videosDaily === "number"
                ? plan.limits.videosDaily
                : null,
            audioWeekly: typeof plan.limits?.audioWeekly === "number"
                ? plan.limits.audioWeekly
                : null,
        };
    });
    if (orderedKeys.length > 0) {
        const missing = ["free", "light", "core", "creator", "professional"].filter((key) => !orderedKeys.includes(key));
        PLAN_ORDER.splice(0, PLAN_ORDER.length, ...orderedKeys, ...missing);
    }
}
function getPaidTiersFromConfig(tierConfig) {
    if (!tierConfig || !Array.isArray(tierConfig.plans))
        return [];
...
function loadUsageStateForUser(userId) {
    ...
    usageState = {
        dayKey: typeof nextState.dayKey === "string" && nextState.dayKey
            ? nextState.dayKey
            : getLocalDayKey(),
        weekKey: typeof nextState.weekKey === "string" && nextState.weekKey
            ? nextState.weekKey
            : getIsoWeekKey(),
...
function toFiniteRatio(used, limit) {
    if (!Number.isFinite(limit) || limit <= 0)
        return 0;
    return Math.min(1, Math.max(0, used / limit));
}
function formatUsageMetric(kind, used, limit) {
...
function updateUsageRow(kind, used, limit) {
...
function renderUsagePanel() {
    ...
    if (usagePlanNameEl)
        usagePlanNameEl.textContent = currentPlanName;
    if (usagePlanMetaEl) {
    ...
    updateUsageRow("cloudChatDaily", usageState.cloudChatDaily, limits.cloudChatDaily);
...
function bumpUsage(kind, amount = 1) {
    pruneUsageState();
...
function isCloudRequest(model, clientUrl) {
    return (lightningEnabled ||
        model === LIGHTNING_MODEL_VALUE ||
        clientUrl === LIGHTNING_CLIENT_URL);
}
function getTierPrice(name) {
    const match = subscriptionTiers.find((tier) => normalizePlanKey(tier.key || tier.name) === normalizePlanKey(name));
    if (!match?.price)
        return null;
    return `$${match.price}/mo`;
}
function getSortedTierCatalog() {
    ...
    return [...subscriptionTiers].sort((a, b) => {
        const aWeight = weight.get(normalizePlanKey(a.key || a.name)) ?? 999;
        const bWeight = weight.get(normalizePlanKey(b.key || b.name)) ?? 999;
        return aWeight - bWeight;
    });
}
function getRecommendedUpgradePlan(kind) {
    ...
    for (let i = Math.max(currentIndex + 1, 0); i < PLAN_ORDER.length; i++) {
        ...
        if (!candidate)
            continue;
        if (PLAN_LIMITS[candidate][kind] == null)
            return candidate;
...
function rememberUpgradeIntent(target = UPGRADE_SETTINGS_TARGET) {
    try {
...
function openUpgradeRequiresAccountDialog(kind) {
    const copy = LIMIT_COPY[kind];
    upgradeModal.open({
        html: `
...
async function openUpgradeDialog(kind) {
    ...
    const plansToShow = tiers.length > 0
        ? tiers
...
function notifyLimitReached(kind) {
    const copy = LIMIT_COPY[kind];
    showNotification({
...
function enforceLimit(kind) {
    if (canConsumeUsage(kind))
        return true;
    notifyLimitReached(kind);
    renderUsagePanel();
    return false;
}
function getPlanNameFromSubscription(info) {
...
function applyUsageData(usage) {
    ...
    const defaultPlanKey = normalizePlanKey(currentTierConfig?.defaultPlanKey || "free");
    ...
    if (!cloud || !images || !videos || !audio)
        return false;
    usageState = {
        dayKey: typeof cloud.window === "string" && cloud.window.trim().length > 0
            ? cloud.window.trim()
            : getLocalDayKey(),
        weekKey: typeof audio.window === "string" && audio.window.trim().length > 0
            ? audio.window.trim()
            : getIsoWeekKey(),
...
async function syncUsageFromServer(renderAfter = true) {
    try {
...
async function refreshSubscriptionData(force = false) {
    ...
    try {
        ...
        const defaultPlanKey = normalizePlanKey(info?.tierConfig?.defaultPlanKey || "free");
        currentPlanPaid = currentPlanKey !== defaultPlanKey;
        if (typeof info?.isPaid === "boolean") {
            currentPlanPaid = Boolean(info.isPaid && currentPlanKey !== defaultPlanKey);
        }
...
function collectUsedAssetIds(sessions) {
    const used = new Set();
    for (const session of Object.values(sessions)) {
        ...
        for (const msg of session.history) {
            if (!msg?.content)
                continue;
            if ((msg.role === "video" || msg.role === "audio") &&
...
function syncLightningToggles(enabled) {
...
function applyLightningState() {
    ...
    if (hostSelect)
        hostSelect.disabled = lightningEnabled;
...
function initLightningToggleEvents() {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    upgradeModal = new window.ic.iModal("upgrade-modal", 700, undefined, false, false);
    initWelcomeCards();
...
function normalizeModelIdForCapabilities(modelValue) {
    return modelValue
...
let modelsSupportsTools = [];
let toolNotice = null;
let modelsSupportsVision = [];
...
async function setToolSupport() {
    ...
    if (modelsSupportsTools.includes(normalizeModelIdForCapabilities(modelSelect.value)) ||
        toolNotice) {
        typingBar.classList.remove("no-tools");
        featureWarning.style.display = "none";
    }
...
function modelSupportsToolsForRequest(modelName) {
    if (modelName === LIGHTNING_MODEL_VALUE || lightningEnabled)
        return true;
    if (toolNotice)
        return true;
    return modelsSupportsTools.includes(normalizeModelIdForCapabilities(modelName));
}
function modelSupportsVisionForRequest(modelName) {
    if (modelName === LIGHTNING_MODEL_VALUE || lightningEnabled)
        return true;
    return modelsSupportsVision.includes(normalizeModelIdForCapabilities(modelName));
}
function setVisionSupport() {
    enforceVisionAttachmentPolicy();
}
function enforceVisionAttachmentPolicy() {
    ...
    renderFileIndicator();
    showNotification({
        message: `Removed ${removedCount} image attachment${removedCount === 1 ? "" : "s"} — the selected model does not support vision.`,
        type: "warning",
    });
}
...
async function loadOptions() {
    ...
    try {
        ...
        try {
            ...
            modelsSupportsTools = (supportsTools || []).map((m) => String(m).toLowerCase());
        }
        catch (e) {
            modelsSupportsTools = [];
            toolNotice =
        ...
        try {
            ...
            modelsSupportsVision = (supportsVision || []).map((m) => String(m).toLowerCase());
        }
        catch (e) {
        ...
        try {
            ...
            modelSelect.insertAdjacentHTML("beforeend", `<option value="add-more-models">➕ Add more models...</option>
        ...
        if (isSyncEnabled() && auth?.session?.isAuthenticated) {
            const remoteResponse = await safeCallRemote(() => window.sync.getRemoteSessions(), { sessions: null });
            ...
            if (!remoteResponse?.error && remoteResponse?.sessions) {
                ...
                sessions = mergeLocalAndRemoteSessions(sessions, remoteResponse.sessions);
                await window.ollama.save(sessions);
        ...
        if (await isOffline()) {
            showNotification({
                message: "⚠️ No internet connection — Using offline sessions only. Tool requests will fail",
                type: "warning",
            });
        }
        else {
...
async function reloadModelsForHost(hostValue) {
    ...
    try {
        ...
        if (err?.code === "UNAUTHORIZED") {
            ...
            showNotification({
                message: "You are not authorized to list models on this remote host.",
                type: "error",
                actions: [
                    {
                        label: "Manage Hosts",
...
function updateToolButtonActiveState() {
...
imgBtn.addEventListener("click", () => {
    if (!imgEnabled && !enforceLimit("imagesDaily"))
        return;
    imgEnabled = !imgEnabled;
    updateToolButtonActiveState();
});
videoBtn.addEventListener("click", () => {
    if (!videoEnabled && !enforceLimit("videosDaily"))
        return;
    videoEnabled = !videoEnabled;
    updateToolButtonActiveState();
});
audioBtn.addEventListener("click", () => {
    if (!audioEnabled && !enforceLimit("audioWeekly"))
        return;
...
let attachMenuDocHandler = null;
let attachMenuKeyHandler = null;
...
function openAttachMenu() {
    ...
    queueMicrotask(() => {
        if (!attachMenuEl || attachMenuEl.classList.contains("hidden"))
            return;
        if (attachMenuDocHandler)
            document.addEventListener("click", attachMenuDocHandler);
        if (attachMenuKeyHandler)
 ...
 */
function buildUserMessageContent(promptText, files) {
...
function formatAttachedTextFiles(files) {
    if (files.length === 0)
        return "";
...
async function autoNameSession(model, // still passed for logging/context
prompt, sessionId, clientUrl) {
...
form.addEventListener("submit", async (e) => {
    ...
    let hostChoice = (hostSelect && hostSelect.value) ||
    ...
    if (droppedImages > 0) {
        showNotification({
            message: `Removed ${droppedImages} image attachment${droppedImages === 1 ? "" : "s"} — the selected model does not support vision.`,
            type: "warning",
        });
    }
    ...
    window.ollama.streamPrompt(model, messageContent, {
    ...
    window.ollama.onError(async (err) => {
        ...
        if (err.toString().toLowerCase().includes("token verification failed")) {
            botBubble.textContent += `\nInferencePort AI Transport Security has rejected the request. Reason: You are not authorized to use this remote host.`;
            showNotification({
                message: "InferencePort AI Transport Security has rejected your request. Reason: You are not authorized to use this remote host.",
                type: "error",
            });
        }
            ...
            showNotification({
                message: "InferencePort AI could not connect to the remote host. Please check the host URL and your network connection.",
                type: "error",
            });
        }
        else {
...
async function persistSessionsAndSync() {
    await window.ollama.save(sessions);
    const auth = await window.auth.getSession();
    if (isSyncEnabled() && auth?.session?.isAuthenticated) {
        await safeCallRemote(() => window.sync.saveAllSessions(sessions));
...
function openEditMessageDialog(messageIndex) {
            ...
            },
            {
                ...
                onClick: async () => {
                    ...
                    if (Array.isArray(existing.content)) {
                        ...
                        const imageParts = existing.content.filter((p) => p.type === "image_url");
...
function openDeleteMessageDialog(messageIndex) {
    editModal.open({
...
function buildMessageActions(msg, messageIndex) {
    ...
    if (canEdit) {
        actions.appendChild(createMessageActionButton("Edit", "Edit message", () => openEditMessageDialog(messageIndex)));
    }
    actions.appendChild(createMessageActionButton("Delete", "Delete message", () => openDeleteMessageDialog(messageIndex)));
    actions.appendChild(createMessageActionButton("Copy", "Copy message", async () => {
...
async function renderChat() {
    ...
    for (const [messageIndex, msg] of session.history.entries()) {
        ...
        if (msg.role === "user") {
            ...
            // nosemgrep: javascript.browser.security.insecure-innerhtml
            bubble.innerHTML = await window.utils.markdown_parse_and_purify(textContent || "");
        ...
        if (msg.role === "assistant") {
            // nosemgrep: javascript.browser.security.insecure-innerhtml
            const html = await window.utils.markdown_parse_and_purify(msg.content || "");
            ...
            Array.from(temp.childNodes).forEach((node) => {
                ...
                if (el.nodeType === Node.ELEMENT_NODE &&
                    ...
                    if (lang === "svg") {
                        ...
                        img.style.cursor = "pointer";
                        img.onclick = () => {
                            const panel = document.getElementById("svg-side-panel");
                            if (panel) {
                                panel.innerHTML = "";
                                ...
                                const xmlLabel = document.createElement("h3");
        ...
        if (msg.role === "tool") {
            ...
            if (msg.name === "generate_image") {
                ...
                if (options) {
                    toolBubble.appendChild(createToolSummaryElement([
                ...
                if (options) {
                    ...
                    if (options.image_urls?.length) {
                        options.image_urls.forEach((item, i) => {
                            summary.appendChild(createToolSummaryRow(`Image ${i + 1}`, createCopyableToolValueElement(item, 100)));
                        });
                    }
                    else {
...
function createImageAssetBubble(dataUrl) {
    const botContainer = document.createElement("div");
...
function createVideoAssetBubble(dataUrl) {
    const botContainer = document.createElement("div");
    botContainer.className = "chat-bubble image-bubble";
...
function createAudioAssetBubble(dataUrl) {
    const botContainer = document.createElement("div");
    botContainer.className = "chat-bubble image-bubble";
...
async function getAssetObjectUrl(assetId, mimeType) {
...
function renderMediaAssetFromContent(role, content, chatBox) {
    if (content.startsWith("data:") || content.startsWith("blob:")) {
        chatBox.appendChild(role === "video"
...
function renderAsset(role, content) {
    const chatBox = document.getElementById("chat-box");
    if (!chatBox)
        return;
    if (role === "image") {
...
function attachAutoImageUpload(fieldWrapper, targetInput) {
    ...
    const renderDefaultHint = () => {
        ...
        link.addEventListener("click", (event) => {
            event.preventDefault();
            if (targetInput.disabled || hiddenFileInput.disabled)
                return;
    ...
    targetInput.addEventListener("paste", async (event) => {
    ...
    targetInput.addEventListener("drop", async (event) => {
        event.preventDefault();
...
function normalizeVideoOptions(raw, source) {
    const obj = raw && typeof raw === "object" ? raw : {};
    const prompt = typeof obj.prompt === "string" ? obj.prompt.trim() : "";
    ...
    const image_urls = source === "user" && Array.isArray(obj.image_urls)
        ? obj.image_urls
...
function getVideoOptionsFromToolCall(call) {
    const toolOptions = normalizeVideoOptions(call?.tool_options, "user");
    if (toolOptions.prompt)
        return toolOptions;
...
function getVideoOptionsFromToolMessage(msg) {
    const payload = safeParseJSON(msg?.content);
    if (payload && typeof payload === "object" && payload.options) {
        const source = payload.status === "resolved" ? "user" : "model";
...
function getImageOptionsFromToolMessage(msg) {
    const payload = safeParseJSON(msg?.content);
    if (payload && typeof payload === "object" && payload.options) {
...
function upsertToolHistoryEntry(session, call, content) {
    if (!Array.isArray(session.history)) {
        session.history = [];
    }
    const index = session.history.findIndex((msg) => msg.role === "tool" && msg.tool_call_id === call.id);
    if (index >= 0) {
...
function setVideoBubbleControlsDisabled(bubble, disabled) {
    bubble.querySelectorAll("textarea,select,input,button").forEach((el) => {
        el.disabled = disabled;
    });
}
function applyVideoOptionsToBubble(bubble, options) {
    const promptEl = bubble.querySelector('[data-video-field="prompt"]');
    const ratioEl = bubble.querySelector('[data-video-field="ratio"]');
    const modeEl = bubble.querySelector('[data-video-field="mode"]');
    const durationEl = bubble.querySelector('[data-video-field="duration"]');
    const image1El = bubble.querySelector('[data-video-field="image-1"]');
    const image2El = bubble.querySelector('[data-video-field="image-2"]');
    if (promptEl)
        promptEl.value = options.prompt || "";
    if (ratioEl)
        ratioEl.value = options.ratio || VIDEO_RATIO_OPTIONS[0];
    if (modeEl)
        modeEl.value = options.mode || VIDEO_MODE_OPTIONS[0];
    if (durationEl)
        durationEl.value = String(options.duration ?? DEFAULT_VIDEO_DURATION);
    if (image1El)
        image1El.value = options.image_urls?.[0] || "";
    if (image2El)
        image2El.value = options.image_urls?.[1] || "";
}
function collectVideoOptionsFromBubble(bubble) {
    const promptEl = bubble.querySelector('[data-video-field="prompt"]');
    const ratioEl = bubble.querySelector('[data-video-field="ratio"]');
    const modeEl = bubble.querySelector('[data-video-field="mode"]');
    const durationEl = bubble.querySelector('[data-video-field="duration"]');
...
function applyImageOptionsToBubble(bubble, options) {
    const promptEl = bubble.querySelector('[data-image-field="prompt"]');
    const modeEl = bubble.querySelector('[data-image-field="mode"]');
    if (promptEl)
        promptEl.value = options.prompt || "";
    if (modeEl)
        modeEl.value = options.mode || IMAGE_MODE_OPTIONS[0];
}
function collectImageOptionsFromBubble(bubble) {
    const promptEl = bubble.querySelector('[data-image-field="prompt"]');
    const modeEl = bubble.querySelector('[data-image-field="mode"]');
...
function applyAudioOptionsToBubble(bubble, options) {
    const promptEl = bubble.querySelector('[data-audio-field="prompt"]');
    if (promptEl)
        promptEl.value = options.prompt || "";
}
function collectAudioOptionsFromBubble(bubble) {
    const promptEl = bubble.querySelector('[data-audio-field="prompt"]');
    return normalizeAudioOptions({
        prompt: promptEl?.value ?? "",
    });
}
function setVideoBubbleStatus(bubble, statusText, isError = false) {
    const statusEl = bubble.querySelector(".video-tool-status");
    if (!statusEl)
        return;
    statusEl.textContent = statusText;
    statusEl.classList.toggle("error", isError);
}
function setLiveToolBubbleState(toolCallId, toolName, state, result) {
    const bubble = liveToolBubbles.get(toolCallId);
    if (!bubble)
        return;
    const label = toolName === "generate_image"
        ? "Image"
...
function createOptionSummaryElement(options) {
...
function createToolSummaryElement(rows) {
    const summary = document.createElement("div");
    summary.className = "video-tool-summary";
    for (const [label, value] of rows) {
...
function createToolSummaryRow(label, value) {
    const row = document.createElement("div");
    row.className = "video-tool-summary-row";
    const labelEl = document.createElement("span");
    labelEl.className = "video-tool-label";
...
function truncateToolValue(value, maxChars = 100) {
    if (value.length <= maxChars)
        return value;
    return `${value.slice(0, maxChars)}...`;
}
function createCopyableToolValueElement(value, maxChars = 100) {
...
function createLiveVideoToolBubble(toolCallId, options) {
    const bubble = document.createElement("div");
    ...
    generateBtn.addEventListener("click", async () => {
        ...
        if (!enforceLimit("videosDaily")) {
            setVideoBubbleStatus(bubble, "Video limit reached for your current plan.", true);
            return;
        }
        setVideoBubbleControlsDisabled(bubble, true);
        setVideoBubbleStatus(bubble, "Submitting video options...");
        try {
            const accepted = await window.ollama.resolveVideoToolCall(toolCallId, selectedOptions);
            if (!accepted) {
                setVideoBubbleControlsDisabled(bubble, false);
                setVideoBubbleStatus(bubble, "Request expired. Try again with a new prompt.", true);
                return;
            }
            ...
            setVideoBubbleControlsDisabled(bubble, false);
            setVideoBubbleStatus(bubble, `Failed to submit options: ${String(err)}`, true);
        }
    });
    cancelBtn.addEventListener("click", async () => {
        ...
        try {
            const accepted = await window.ollama.resolveVideoToolCall(toolCallId, null);
            if (!accepted) {
                setVideoBubbleStatus(bubble, "Request expired. Try again with a new prompt.", true);
                return;
            }
            ...
            setVideoBubbleControlsDisabled(bubble, false);
            setVideoBubbleStatus(bubble, `Failed to cancel request: ${String(err)}`, true);
        }
    });
...
function createLiveImageToolBubble(toolCallId, options) {
    const bubble = document.createElement("div");
    bubble.className = "chat-bubble tool-bubble video-tool-bubble";
    bubble.setAttribute("data-tool-call-id", toolCallId);
    const header = document.createElement("div");
    ...
    generateBtn.addEventListener("click", async () => {
        ...
        if (!enforceLimit("imagesDaily")) {
            setVideoBubbleStatus(bubble, "Image limit reached for your current plan.", true);
            return;
        }
        setVideoBubbleControlsDisabled(bubble, true);
        setVideoBubbleStatus(bubble, "Submitting image options...");
        try {
            const accepted = await window.ollama.resolveImageToolCall(toolCallId, selectedOptions);
            if (!accepted) {
                setVideoBubbleControlsDisabled(bubble, false);
                setVideoBubbleStatus(bubble, "Request expired. Try again with a new prompt.", true);
                return;
    ...
    cancelBtn.addEventListener("click", async () => {
        ...
        try {
            const accepted = await window.ollama.resolveImageToolCall(toolCallId, null);
            if (!accepted) {
                setVideoBubbleStatus(bubble, "Request expired. Try again with a new prompt.", true);
                return;
            }
...
function createLiveAudioToolBubble(toolCallId, options) {
    const bubble = document.createElement("div");
    bubble.className = "chat-bubble tool-bubble video-tool-bubble";
    bubble.setAttribute("data-tool-call-id", toolCallId);
    const header = document.createElement("div");
    ...
    generateBtn.addEventListener("click", async () => {
        ...
        if (!enforceLimit("audioWeekly")) {
            setVideoBubbleStatus(bubble, "Audio limit reached for your current plan.", true);
            return;
        }
        setVideoBubbleControlsDisabled(bubble, true);
        setVideoBubbleStatus(bubble, "Submitting audio prompt...");
        try {
            const accepted = await window.ollama.resolveAudioToolCall(toolCallId, selectedOptions);
            if (!accepted) {
                setVideoBubbleControlsDisabled(bubble, false);
                setVideoBubbleStatus(bubble, "Request expired. Try again with a new prompt.", true);
                return;
            ...
            setVideoBubbleControlsDisabled(bubble, false);
            setVideoBubbleStatus(bubble, `Failed to submit prompt: ${String(err)}`, true);
        }
    });
    const cancelBtn = document.createElement("button");
    cancelBtn.type = "button";
    ...
    cancelBtn.addEventListener("click", async () => {
        ...
        try {
            const accepted = await window.ollama.resolveAudioToolCall(toolCallId, null);
            if (!accepted) {
                setVideoBubbleStatus(bubble, "Request expired. Try again with a new prompt.", true);
                return;
            }
...
window.ollama.onNewAsset((msg) => {
    ...
    const content = msg.role === "image" && !msg.content.startsWith("data:")
        ? `data:image/png;base64,${msg.content}`
        : msg.content;
    if (last?.role === msg.role && last?.content === content)
        return;
    if (["image", "video", "audio"].includes(last?.role) &&
...
window.ollama.onToolCall((call) => {
    ...
    if (call.name === "generate_video") {
        ...
        if (shouldRender) {
            ...
            applyVideoOptionsToBubble(bubble, options);
            setLiveToolBubbleState(call.id, call.name, call.state, typeof call.result === "string" ? call.result : undefined);
            if (autoScroll)
                chatBox.scrollTop = chatBox.scrollHeight;
        }
        void window.ollama.save(sessions);
    ...
    if (call.state === "resolved") {
        upsertToolHistoryEntry(session, call, typeof call.result === "string" ? call.result : "✅ Done");
        void window.ollama.save(sessions);
        return;
    }
    if (call.state === "canceled") {
        upsertToolHistoryEntry(session, call, typeof call.result === "string"
            ? call.result
...
function openAddHostDialog() {
    ...
    document
        ...
        ?.addEventListener("click", () => {
        let url = document.getElementById("new-host-url").value.trim();
        const alias = document.getElementById("new-host-alias").value
            .trim()
            .substring(0, 20);
        if (!url)
            return;
