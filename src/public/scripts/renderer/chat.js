...
async function autoNameSession(model, prompt, sessionId) {
    ...
    const response = await fetch("http://localhost:11434/api/chat", {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "system",
                    content: "Name this session based on the user prompt that the user submits. Only respond with the title.",
                },
                {
                    role: "user",
    ...
    try {
        ...
        showNotification({
            message: "Failed to auto-name session: invalid response from model API.",
            type: "error",
        });
        return new Date().toLocaleString();
    }
    if (!data || !data.message || typeof data.message.content !== "string") {
        ...
        showNotification({
            message: "Failed to auto-name session: unexpected response from model API.",
            type: "error",
        });
        return new Date().toLocaleString();
    }
