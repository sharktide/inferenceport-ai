...
async function renderWebsites() {
    const spinner = document.getElementById("spinner-website");
    spinner.style.display = "flex";
    ...
    function handleDeleteClick() {
        if (type === "ollama") {
            try {
                ...
                const card = document.querySelector(`.marketplace-card[modelid="${username}"]`);
                if (card) {
            ...
            try {
                ...
                const card = document.querySelector(`.marketplace-card[spaceid="${spaceId}"]`);
                if (card) {
                    card.style.transition = "opacity 0.4s ease, transform 0.4s ease";
                    card.style.opacity = "0";
                    card.style.transform = "scale(0.95)";
            ...
            try {
                ...
                const card = document.querySelector(`.marketplace-card[siteId="${siteID}"]`);
                if (card) {
                    card.style.transition = "opacity 0.4s ease, transform 0.4s ease";
                    card.style.opacity = "0";
                    card.style.transform = "scale(0.95)";
...
function shareSpace(username, repo) {
    window.hfspaces.share(username, repo);
}
function shareWebsite(url, title) {
    window.hfspaces.share_website(url, title);
}
window.showDelModal = showDelModal;
