...
signupButton.addEventListener("click", async () => {
	if ((window as any).mode === 0) {
		...
		if (!email || !password)
			return updateStatus(
				"Missing credentials (email + password required)",
			);
		...
		updateStatus(
			`Account created for ${email}. Please check your email to confirm before logging in.`,
		);
	} else if ((window as any).mode === 1) {
		const email = emailInput.value.trim();
		if (!email)
...
function forgotPassword() {
	...
	emailInput.value = "";
	(
		document.getElementById("social-buttons") as HTMLDivElement
	).style.display = "none";
	(document.getElementById("rst") as HTMLTitleElement).style.display =
		"block";
