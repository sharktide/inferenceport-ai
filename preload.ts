...
contextBridge.exposeInMainWorld('ollama', {
  ...
  resetChat: (): Promise<void> =>
    ipcRenderer.invoke('ollama:reset'),
  ...
  onPullProgress: (cb: (data: PullProgress) => void): void =>
    ipcRenderer.on('ollama:pull-progress', (__: Electron.IpcRendererEvent, data: PullProgress) => cb(data)),
  ...
  onResponse: (cb: (token: string) => void): void =>
    ipcRenderer.on('ollama:chat-token', (__: Electron.IpcRendererEvent, token: string) => cb(token)),

  onError: (cb: (err: string) => void): void =>
    ipcRenderer.on('ollama:chat-error', (__: Electron.IpcRendererEvent, err: string) => cb(err)),

  onDone: (cb: () => void): void =>
    ipcRenderer.on('ollama:chat-done', () => cb()),

  load: (): Promise<Sessions> =>
    ipcRenderer.invoke('sessions:load'),

  save: (sessions: Sessions): Promise<void> =>
    ipcRenderer.invoke('sessions:save', sessions),

  getPath: (): Promise<string> =>
    ipcRenderer.invoke('session:getPath'),
