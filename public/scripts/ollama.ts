...
// Utility Functions
function showNotification({ message, type = "info", actions = [] }: NotificationOptions): void {
...
//@ts-ignore
window.ollama.onPullProgress(({ model, output }: { model: string; output: string }) => {
  const container = document.getElementById("notification-container");
  if (!container) return;

  let box = container.querySelector(`[data-model="${model}"]`) as HTMLElement | null;
  ...
  if (pre) {
    pre.textContent = clean.includes("\r") ? clean.split("\r").pop() ?? "" : clean;
  }
...
document.getElementById("search-installed")?.addEventListener("input", (e: Event) => {
  const target = e.target as HTMLInputElement;
  renderInstalledModels(target.value);
});

document.getElementById("search-available")?.addEventListener("input", (e: Event) => {
  const target = e.target as HTMLInputElement;
