...
function isSafeObjectKey(key) {
	return (
		typeof key === "string" &&
		/^[a-zA-Z0-9_-]+$/.test(key) &&
		!DISALLOWED_OBJECT_KEYS.has(key)
	);
}
...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (!isSafeObjectKey(rawKey)) {
			throw new Error(`Invalid argument name: ${rawKey}`);
		}
...
function main() {
	...
	if (workspaceRoot) {
		...
		if (relative.startsWith("..") || path.isAbsolute(relative)) {
			throw new Error(
				`directory must stay within workspace_root (${resolvedRoot})`,
			);
		}
	}
