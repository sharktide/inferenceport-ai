...
async function verifyToken(token: string, secret: string): Promise<{ ok: boolean, id?: string }> {
...
Deno.serve(async (req) => {
  ...
  const token = match[1];
  const { ok, id: cookieUserId } = await verifyToken(token, Deno.env.get("COOKIE_SECRET")!);
  if (!ok || !cookieUserId) {
    return new Response(JSON.stringify({ error: "Verification required" }), { status: 403 });
  }
